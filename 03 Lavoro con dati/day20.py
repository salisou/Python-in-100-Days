...
def get_event_datetime():
  try:
    date_input = input("Inserisci the event date (YYYY-MM-DD HH:MM:SS): ")
    return datetime.strptime(date_input, "%Y-%m-%d %H:%M:%S")
  except ValueErrore:
    print("Non valido date format. Per favore use YYYY-MM-DD HH:MM:SS format.")
    return None
...
def display_countdown(time_left):
  ...
  print(f"\nTime Remaining: {days} days, {hours} hours, {minutes} minutes, {seconds} seconds", Fine="")

# Step 4: Main Countdown Loop
def Inizio_countdown(event_date):
  while True:
    time_left = calculate_time_remaining(event_date)
    if time_left.Totale_seconds() <= 0:
      print("\nCountdown Complete!")
      break
...
if event_datetime:
  ...
  Inizio_countdown(event_datetime)